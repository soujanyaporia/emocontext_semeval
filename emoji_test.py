...
def getEmbeddingMatrix(wordIndex, EMBEDDING_DIM):
    ...
    unk_word = np.random.uniform(0, 1, 300)
    ...
    for word, i in wordIndex.items():
        ...
        if embeddingVector is not None:
            ...
            print('Found vec for ', word)
            embeddingMatrix[i] = emoji_model[word]
        else:
            embeddingMatrix[i] = np.random.uniform(0, 1, 300)

    vocab_len = embeddingMatrix.shape[0]
    emb_size = embeddingMatrix.shape[1]
    exit(0)

    return embeddingMatrix, vocab_len, emb_size


def getEmbeddingMatrix1(wordIndex, EMBEDDING_DIM):
    """Populate an embedding matrix using a word-index. If the word "happy" has an index 19,
       the 19th row in the embedding matrix should contain the embedding vector for the word "happy".
    Input:
        wordIndex : A dictionary of (word : index) pairs, extracted using a tokeniser
    ...
    unk_word = np.random.uniform(0, 1, 300)
    # print(unk_word.shape)
    model = gensim.models.KeyedVectors.load_word2vec_format(
        os.path.join(os.path.dirname(__file__), 'glove.840B.300d.w2vformat.txt'), binary=False)
    print('Found %s word vectors.' % len(model.wv.vocab))
    # Minimum word index of any word is 1.
    embeddingMatrix = np.zeros((len(wordIndex) + 1, EMBEDDING_DIM))
    for word, i in wordIndex.items():
...
def main():
    ...
    # for word, i in wordIndex.items():
    #     print(word.split(), '\t\t\t', str(word.encode("utf-8")), '\t\t\t')

    print("Found %s unique tokens." % len(wordIndex))
    exit()
    print("Populating embedding matrix...")
    embeddingMatrix, _, _ = getEmbeddingMatrix(wordIndex, EMBEDDING_DIM)
