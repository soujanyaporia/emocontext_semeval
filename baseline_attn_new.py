...
def buildModel(embeddingMatrix):
    ...
    lstm = Bidirectional(LSTM(LSTM_DIM, dropout=DROPOUT, return_sequences=False))

    # attention = Attention(MAX_SEQUENCE_LENGTH) # comment it out when you use attention

    # lstm1 = attention(lstm(emb1)) # used for attention
    # lstm2 = attention(lstm(emb2))
    # lstm3 = attention(lstm(emb3))
    ...
    lstm_up = (LSTM(2 * LSTM_DIM, dropout=DROPOUT))

    out = lstm_up(inp)
    out = BatchNormalization()(out)

    # out = Dense(LSTM_DIM*2)(out)
