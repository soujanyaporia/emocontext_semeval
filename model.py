...
class LSTM_Model():
    def __init__(self, input_shape, lr, vocab_size, embedding_dim, emotions,
                 enable_attn_2=False, seed=1234):

        self.input1 = tf.placeholder(dtype=tf.int64, shape=(None, input_shape))
        self.input2 = tf.placeholder(dtype=tf.int64, shape=(None, input_shape))
        self.input3 = tf.placeholder(dtype=tf.int64, shape=(None, input_shape))

        self.emotions = emotions
        self.seq_len1 = tf.placeholder(tf.int32, [None, ], name="seq_len1")
        self.seq_len2 = tf.placeholder(tf.int32, [None, ], name="seq_len2")
        self.seq_len3 = tf.placeholder(tf.int32, [None, ], name="seq_len3")
        self.mask = None
        self.y = tf.placeholder(tf.int32, [None, self.emotions], name="y")
        self.lr = lr
        self.seed = seed
    ...
    def GRU(self, inputs, output_size, name, dropout_keep_rate):
        with tf.variable_scope('rnn_' + name, reuse=tf.AUTO_REUSE):
            ...
            output, state = tf.nn.dynamic_rnn(cell, inputs, sequence_length=None, dtype=tf.float32)

            return output, state


    def BiGRU(self, inputs, output_size, name, dropout_keep_rate, seq_len):
        with tf.variable_scope('rnn_' + name, reuse=tf.AUTO_REUSE):
            kernel_init = tf.glorot_uniform_initializer(seed=self.seed, dtype=tf.float32)
            bias_init = tf.zeros_initializer()
            ...
            outputs, states = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_cell, cell_bw=fw_cell, inputs=inputs,
                                                              sequence_length=seq_len, dtype=tf.float32)

            output_fw, output_bw = outputs
            output = tf.concat([output_fw, output_bw], axis=-1)
            output_st, output_st = states
            state = tf.concat([output_st, output_st], axis=-1)
            return output, state
    ...
    def self_attention_2(self, inputs, name):
        ...
        with tf.variable_scope(scope_name, reuse=tf.AUTO_REUSE):
            ...
            print('self_attention_2: final_output', final_output.get_shape())
            return final_output
    ...
    def _build_model_op(self):
        ...
        input1 = tf.nn.embedding_lookup(self.W, self.input1)
        input2 = tf.nn.embedding_lookup(self.W, self.input2)
        input3 = tf.nn.embedding_lookup(self.W, self.input3)
        print('input1', input1.get_shape())
        print('input2', input2.get_shape())
        print('input3', input3.get_shape())

        _, input1 = self.BiGRU(input1, 300, 'gru', 1 - self.lstm_dropout, self.seq_len1)
        _, input2 = self.BiGRU(input2, 300, 'gru', 1 - self.lstm_dropout, self.seq_len2)
        _, input3 = self.BiGRU(input3, 300, 'gru', 1 - self.lstm_dropout, self.seq_len3)
        print('GRU: input3', input3.get_shape())

        input1 = tf.expand_dims(input1, 1)
        input2 = tf.expand_dims(input2, 1)
        input3 = tf.expand_dims(input3, 1)

        input = tf.concat([input1, input2, input3], axis=1)
        print('input', input.get_shape())

        # input = tf.nn.dropout(input, 1-self.lstm_inp_dropout)
        self.gru_output, state = self.GRU(input, 300, 'gru2', 1 - self.lstm_dropout)
        # self.inter = tf.nn.dropout(self.gru_output, 1 - self.dropout_lstm_out)
        if self.attn_2:
            self.inter = self.gru_output
            print('self.attn_2 enabled: ', self.attn_2)
            self.inter = self.self_attention_2(self.inter, '')
            print('self.inter', self.inter.get_shape())
        ...
        self.inter1 = Dense(50, activation=tf.nn.relu,
